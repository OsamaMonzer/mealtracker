...
export default function RecipeBuilder() {
  ...
  async function saveRecipe() {
    ...
    setSaving(true);
    const res = await fetch('/api/recipes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: recipeName, portions: parseInt(portions) || 1, ingredients: ings }) });
    setSaving(false);
      ...
      <div className="page-header" style={{ marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <Link href="/" style={{ textDecoration: 'none', color: 'inherit' }}>
            <h1 className="page-title animate-fade-up" style={{ fontSize: '3rem' }}>New Recipe</h1>
          </Link>
        </div>
      </div>
